<?php
	...
	$query = "INSERT INTO votimi_umks (ID_Pyetja,ID_Pergjigjja,ID_Pjesmarresi) VALUES ('" . $ID_Pyetja ."','" . $ID_Pergjigjja . "','" . $_SESSION["ID_Pjesmarresi"] . "')";
    $insert_id = $kontrolluesidb->insertQuery($query);
    
    if(!empty($insert_id)) {
        $query = "SELECT * FROM pergjigjet_umks WHERE ID_Pyetja = " . $ID_Pyetja;
...
<?php
        $query = "SELECT count(ID_Pergjigjja) as total_count FROM votimi_umks WHERE ID_Pyetja = " . $ID_Pyetja;
        $total_rating = $kontrolluesidb->runQuery($query);

        foreach($Pergjigjja as $k=>$v) {
            $query = "SELECT count(ID_Pergjigjja) as Pergjigjja_count FROM votimi_umks WHERE ID_Pyetja = " .$ID_Pyetja . " AND ID_Pergjigjja = " . $Pergjigjja[$k]["ID"];
